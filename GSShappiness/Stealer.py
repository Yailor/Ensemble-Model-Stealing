...
class Stealer():
    ...
    def load_augmentation_data(self, target):
        ...
        self.data = pd.read_csv("data/gss_augmentation_data-{}%.csv".format(self.proportion))
    ...
    def data_augmentation(self, target):
        ...
        '''save as csv'''
        sample_pool.to_csv("data/gss_augmentation_data-5%.csv", index=False)
    ...
    def train_base_clf(self, data, label):
        tree = DecisionTreeClassifier(criterion='gini', random_state=60, class_weight='balanced', max_depth=80)
        tree.fit(data, label)
