...
from adult.Stealer import Stealer
from adult.Target import Target
import pandas as pd
import random
import numpy as np
...
def prepare(target, stealer):
    ...
    stealer.row = step
    target_pred_train = target.predict(h0_train)
    ...
    stealer.row += step  # "="
    target_pred_test = target.predict(h0_test)
...
def test_accu(stealer, target):
    X = target.test.drop(['income'], axis=1)
    y = target.test['income']
    pred = stealer.forest_predict(X, rows=len(X))
    target_pred = target.predict(X)
    #print(pred)
    #print(target_pred)
    return accuracy_score(target_pred, pred), accuracy_score(y, pred)
...
if __name__ == '__main__':
    ...
    print("proportion of training data:{}%".format(stealer.proportion))
    ...
    print("proportion of training data:{}".format(stealer.proportion))
    while stealer.row < (pool_len-700):
        if i >= 1:
    ...
    plt.title('{} extraction on adult-{}%'.format(tar_type, stealer.proportion))
    ...
    #test
    aug_pred_s = stealer.forest_predict(stealer.train_aug_x, rows=len(stealer.train_aug_x))
    #aug_pred_t = target.predict(stealer.train_aug)
    print("accuracy on augmentation data:{}".format(accuracy_score(aug_pred_s, stealer.train_aug_y)))

    '''save submodels'''
    for i in range(len(stealer.models)):
        joblib.dump(stealer.models[i], 'model/{}.pkl'.format(i))
