...
class Stealer():
    ...
    def load_augmentation_data(self, target):
        ...
        self.train_aug_x = pd.read_csv('data/chosen augmentation data-{}%.csv'.format(self.proportion))
        self.data = pd.read_csv("data/adult_samplepool-{}%.csv".format(self.proportion), index_col=0)


    def data_augmentation(self, target):
        train = target.train #including label
        test = target.test
        num = len(train)
        self.train_aug = train.sample(n=int(num*self.proportion/100))
        self.train_aug_y = self.train_aug['income']
        self.train_aug_x = self.train_aug.drop(['income'], axis=1)
        self.test = target.test
        sample_pool = self.train_aug_x
        '''save aug data to csv'''
        self.train_aug_x.to_csv('data/chosen augmentation data-{}%.csv'.format(self.proportion))
        '''augmentation procedure'''
        '''1.get all types for each column'''
        feature_list = []

        for col in self.train_aug_x.iteritems():
        ...
        '''save as csv'''
        sample_pool.to_csv("data/adult_samplepool-{}%.csv".format(self.proportion))
        return sample_pool
    ...
    def active_learning(self, clf, random_pool, target, err_set, tar_lb):
        ...
        while i <= len(random_pool):
            ...
            xi_list = xi.tolist()
            xi_frame = pd.DataFrame(columns=xi.axes)
            xi_frame.loc[len(xi_frame)] = xi_list
            #xgb_xi = xgb.DMatrix(xi_frame)
        ...
        if len(S):
            ...
            for i in range(len(S)):
                ...
                if S_leaf_id == leaf_id:
                    clf_pred_pt = self.change_label(clf_pred_pt)
