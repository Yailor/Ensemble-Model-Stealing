...
class Stealer():
    ...
    def load_pool(self):
        data = pd.read_csv("data/fake_aug_pool/fake-{}%/fake-{}%.csv".format(self.proportion, self.proportion), encoding='latin-1')\
                            .dropna(axis=0, how='any').sample(frac=1)
        for i in range(20):
            """delete Nan rows"""
            data = data.append(pd.read_csv("data/fake_aug_pool/fake-{}%/fake_aug_pool{}.csv"
                                           .format(self.proportion, i)).dropna(axis=0, how='any').sample(frac=1))
