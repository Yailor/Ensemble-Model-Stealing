...
from fake_news.Stealer import Stealer
from fake_news.Target import Target
import pandas as pd
import random
import numpy as np
...
def test_accu(stealer, target):
    ...
    test = pd.read_csv("data/fake_aug_pool/processed_news.csv").dropna(axis=0, how='any')
    true_lb = test['target'][:5000]
    test = test['text'][:5000]
    test_vector = target.vectorizer.transform(test)
    target_pred = target.predict(test_vector).tolist()
    pred = stealer.forest_predict(test_vector, rows=5000)
    #pred = list(map(int, pred))   #transform from str to int
...
def form_err_rgt_set(target, stealer, rd):
    ...
    while j >= (rd - 3) and j >= 0:
        # err set
        err_data = pd.read_csv("err_set/err_set{}.csv".format(j)).dropna(axis=0, how='any')
        ...
        # rgt set
        rgt_data = pd.read_csv("rgt_set/rgt_set{}.csv".format(j)).dropna(axis=0, how='any')
...
def test_aug_data(stealer, target):
    data = pd.read_csv("data/fake_aug_pool/fake-{}%/fake-{}%.csv".format(stealer.proportion, stealer.proportion),
                       encoding='latin-1') \
        .dropna(axis=0, how='any').sample(frac=1)
    X = data['text']
    y = data['target']
...
if __name__ == '__main__':
    ...
    print("pro=".format(stealer.proportion))
    ...
    # while i < 200:
        ...
        add_random = stealer.data[stealer.row : stealer.row+step].dropna(axis=0, how='any')
        stealer.row += step
        ...
        if abs(target.accuracy - accu2) <= 0.01 and i >= 10: break
        i += 1

    # print hyperparameters
    plt.xlabel('queries')
    plt.ylabel('agreement')
    plt.title('{} extraction on fake-{}%'.format(tar_type, stealer.proportion))
